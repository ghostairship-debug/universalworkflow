...
- 2026-05-04 visibility ratchet: high-risk commercial task cards using `human_visible_cli_enforced` now execute with `control_plane_visibility=resident` and `provider_visibility=direct_visible` for `codex` and `opencode`. The resident control plane records mirrored control logs and DB heartbeat evidence; only the inner provider subprocess opens a human-visible, human-readable console, with provider-level `stdout.log`, `stderr.log`, `stream.jsonl`, and `visible_cli_session.json` recorded under the task-card visible-session directory. Missing direct-provider session metadata blocks the commercial implementation card.
...
- Next design direction is generalized beyond this block-puzzle sample: [UNIVERSAL_GAME_PRODUCTION_AI_PLAYTEST_UPGRADE_PLAN_2026_05_03.md](UNIVERSAL_GAME_PRODUCTION_AI_PLAYTEST_UPGRADE_PLAN_2026_05_03.md) defines the new universal game-production quality floor and AI surrogate playtest architecture.
- Implementation progress: Round 1 through Round 8 of the universal plan are now implemented at the contract/workflow-callable/pipeline-materialization layer. New modules add `GameDesignSpec`, derived-only semantic enrichment, engine-native product-body rejection for browser/canvas/runtime-hook substitutes, AI surrogate quality scoring, AI findings-to-repair task cards, AI playtest plan generation, GameDesignSpec-to-current-phase workflow task-card generation, AI playtest runner/execution packet validation, and `workflowctl game` commands for IR, derived-only enrichment, AI playtest plans, production cards, AI playtest runs, AI execution gates, AI quality gates, AI repair cards, and AI NO-GO repair loops. The `commercial_game_production` task-card generation agent now recognizes `Universal Game Production Quality And AI Playtest Architecture` and materializes only the current phase's DB task cards, split by source requirement category rather than collapsed into one broad product card. `commercial_game_production` now requires AI surrogate playtest evidence as a default commercial machine gate, and `workflowctl test matrix` includes both the universal game / AI playtest regression set and a deeper `commercial_full` suite. These are still architecture/control-plane capabilities, not a new commercial game content phase.
