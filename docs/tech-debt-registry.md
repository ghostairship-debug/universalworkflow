...
| ID | 描述 | 当前状态 | 阻塞影响 |
| --- | --- | --- | --- |
| M77-COCOS-001 | Cocos pipeline 已有 CLI/E2E/build/playtest、trusted Editor bridge、same-project task-card worker、真实资产链路、product-body/runtime、asset graph、browser interaction、audio runtime evidence，以及最新中文 UI/BGM/拖拽/面板修复 | agent_qa_repaired_awaiting_human_acceptance | 阻塞声明 `commercial_playable_go=true`；agent QA 通过只能支持复查，最终商业 GO 必须等待真实人工玩家验收 |
| M84-GAME-QA-001 | 商业化验收 gate 曾误收 runtime hook / canvas / event coverage；post-review hardening 已修复该机器假阳性路径并区分 machine-ready、agent QA 与人审接受 | agent_qa_passed_but_human_gate_open | 阻塞任何商业 GO；只有人工玩家验收可解除 `awaiting_human_player_review` |
| M109-NO-DEGRADATION-COMMERCIAL-001 | `pipeline_ecf26665254e` 曾被机器证据接受但真实人审判定产物不是完整游戏本体；后续同项目实现、product-depth、asset、browser、audio runtime proof 和最新中文/BGM/拖拽修复已用玩家可见证据补齐 | agent_qa_repaired_awaiting_human_acceptance | 直接阻塞 `commercial_playable_go=true` 直到人工验收接受；不再阻塞继续复查当前修复 build |
