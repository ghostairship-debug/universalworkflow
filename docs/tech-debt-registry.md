...
| ID | 描述 | 当前状态 | 阻塞影响 |
| --- | --- | --- | --- |
| M77-COCOS-001 | Cocos pipeline 已有 CLI/E2E/build/playtest、trusted Editor bridge、same-project task-card worker、真实资产链路、product-body/runtime、asset graph、browser interaction 和 audio runtime evidence | machine_evidence_go_awaiting_human_review | 仅阻塞声明 `commercial_playable_go=true`；机器证据已 GO，最终商业 GO 必须等待真实人工玩家验收 |
| M84-GAME-QA-001 | 商业化验收 gate 曾误收 runtime hook / canvas / event coverage；post-review hardening 已修复该机器假阳性路径并区分 machine-ready 与人审接受 | machine_gate_false_positive_repaired_awaiting_human_review | 阻塞任何商业 GO；当前机器证据已过，只有人工玩家验收可解除 `awaiting_human_player_review` |
| M109-NO-DEGRADATION-COMMERCIAL-001 | `pipeline_ecf26665254e` 曾被机器证据接受但真实人审判定产物不是完整游戏本体；后续同项目实现、product-depth、asset、browser、audio runtime proof 已用玩家可见、非 runtime-hook 硬证据补齐 | machine_evidence_go_awaiting_human_review | 直接阻塞 `commercial_playable_go=true` 直到人工验收接受；不再阻塞 `machine_evidence_go=true` |
