...
def commercial_game_production_stages(pipeline_id: str = REAL_COMMERCIAL_GAME_PIPELINE_ID) -> list[PipelineStage]:
    ...
    multimodal = _role_stage(
        ...
        pipeline_id=pipeline_id,
        depends_on=[tech.stage_id],
    )
    task_cards = _role_stage(
        name="Task card generation agent",
        order_index=5,
        goal="Prepare high-quality task card inputs with read/write sets, tests, evidence, blockers, and model guidance.",
        role_id="task_card_generation_agent",
        pipeline_id=pipeline_id,
        depends_on=[multimodal.stage_id],
    )
    ...
    return [intake, product, ui, tech, multimodal, task_cards, asset_generation, implementation, qa, supervisor, readiness_gate]
