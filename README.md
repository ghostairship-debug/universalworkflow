...
## Current Document Authority (2026-05-08)

Use these documents in this order for commercial game pipeline work:
...
- Current no-downgrade repair plan: [COMMERCIAL_GAME_PIPELINE_NO_DOWNGRADE_REPAIR_PLAN_2026_05_07.md](COMMERCIAL_GAME_PIPELINE_NO_DOWNGRADE_REPAIR_PLAN_2026_05_07.md). It records the 2026-05-07 weak PDF-only regeneration as a product-quality NO-GO, hardens the `GameDesignSpec` / active-phase blueprint / AI surrogate gate path, and adds the 2026-05-08 same-project repair-loop closure.
...
- 2026-05-07 de-templating update: commercial Cocos bootstrap now creates a generic empty shell only; it no longer installs the block-puzzle product-body baseline or post-build browser runtime bridge by default. Game-specific execution now flows from lossless `GameDesignSpec` to agent-proposed product phase candidates, active-phase `PhaseExecutionBlueprint`, workflow-compiled DB task cards, and spec-driven semantic/product-body evidence contracts. Block-puzzle terms such as `10x10`, `CandidateTray`, and `anti_stall` are allowed only when the current source spec requires them, otherwise they are treated as template leakage.
